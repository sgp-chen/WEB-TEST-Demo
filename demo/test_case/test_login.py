...
class login_(unittest.TestCase):
    ...
    def test_07(self):
        ...
        self.um.click_ok()
        try:
            user = self.driver.find_element_by_link_text("更改成功")
            self.assertEqual(user.text, "更改成功", msg="测试不通过")
            logger.info("员工离职成功")
        except:
            user = self.driver.find_element_by_link_text("该员工有未完成任务")
            self.assertEqual(user.text, "该员工有未完成任务", msg="测试不通过")
            logger.debug("员工有任务离职失败")
